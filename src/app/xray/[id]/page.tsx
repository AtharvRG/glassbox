...
const FilterEvaluationsTable = ({ evaluations }: { evaluations: any[] }) => {
  ...
  return (
    <div className="mt-4 space-y-3">
      {sorted.map((item, idx) => (
        <div key={idx} className={`rounded-xl border overflow-hidden ${item.qualified ? 'bg-inch-worm/5 border-inch-worm/20' : 'bg-red-500/5 border-red-500/30'}`}>
          <div className={`px-4 py-3 flex items-center justify-between ${item.qualified ? 'bg-inch-worm/10' : 'bg-red-500/10'}`}>
            <div className="flex items-center gap-3">
              {item.qualified ? <Check className="h-5 w-5 text-inch-worm" /> : <X className="h-5 w-5 text-red-400" />}
              <div>
            ...
            <span className={`text-xs font-bold px-2 py-1 rounded ${item.qualified ? 'bg-inch-worm/20 text-inch-worm' : 'bg-red-500/20 text-red-400'}`}>
          ...
          {item.filter_results && (
            <div className="px-4 py-2 border-t border-white/5 flex gap-2 flex-wrap">
              {Object.entries(item.filter_results).map(([name, result]: [string, any]) => (
                <span key={name} className={`text-xs px-2 py-1 rounded ${result.passed ? 'bg-inch-worm/10 text-inch-worm' : 'bg-red-500/10 text-red-400'}`}>
...
const LLMRelevanceDisplay = ({ output }: { output: any }) => {
  ...
  const getMatchColor = (type: string) => {
    switch (type) {
      case 'TRUE_MATCH': return 'bg-inch-worm/10 border-inch-worm/30 text-inch-worm';
  ...
  const getMatchIcon = (type: string) => {
    switch (type) {
      case 'TRUE_MATCH': return <ThumbsUp className="h-5 w-5 text-inch-worm" />;
  ...
  return (
    <div className="space-y-4">
      {output.user_query && (
        <div className="bg-mosque/30 border border-inch-worm/20 rounded-lg p-3">
          <span className="text-xs text-inch-worm font-medium">User searched for: </span>
          <span className="text-sm text-white font-semibold">&quot;{output.user_query}&quot;</span>
        </div>
      )}
      {output.summary && (
        <div className="flex gap-3">
          <div className="flex-1 bg-inch-worm/10 rounded-lg p-3 text-center border border-inch-worm/20">
            <div className="text-xl font-bold text-inch-worm">{output.summary.true_matches}</div>
            <div className="text-xs text-inch-worm/70">True Matches</div>
          </div>
      ...
      <div className="space-y-3">
        <h4 className="text-xs font-bold text-white/50 uppercase tracking-wider">LLM Relevance Analysis</h4>
        {(output.ranked_list || output.evaluations).map((item: any, idx: number) => (
          <div key={idx} className={`rounded-xl border overflow-hidden ${getMatchColor(item.match_type)}`}>
            ...
            <div className="px-4 py-3 border-t border-white/10">
              <div className="flex gap-2">
                <Sparkles className="h-4 w-4 text-inch-worm flex-shrink-0 mt-0.5" />
                <div>
                  <span className="text-xs font-bold text-inch-worm">LLM ANALYSIS: </span>
                  <span className="text-sm text-white/80">{item.reason}</span>
                </div>
              </div>
            </div>
          </div>
...
const FiltersAppliedCard = ({ filters }: { filters: any }) => {
  ...
  return (
    <div className="mb-4 bg-amber-500/10 border border-amber-500/20 rounded-lg p-3">
      <h4 className="text-xs font-bold text-amber-300 uppercase tracking-wider mb-2">Filters Applied</h4>
      <div className="flex flex-wrap gap-2">
        {Object.entries(filters).map(([key, value]: [string, any]) => (
          <span key={key} className="text-xs bg-amber-500/10 text-amber-200 px-2 py-1 rounded border border-amber-500/20">
            {value.rule}
          </span>
        ))}
      </div>
...
export default async function TraceDetail({ params }: { params: { id: string } }) {
  const { data: execution } = await supabase.from('executions').select('id, name, status, created_at, metadata').eq('id', params.id).single();
  const { data: steps } = await supabase.from('steps').select('id, step_name, step_order, input, output, reasoning, status, duration_ms').eq('execution_id', params.id).order('step_order', { ascending: true });
  ...
  return (
    <div className="min-h-screen flex flex-col items-center pb-20 relative overflow-hidden">
      <div className="absolute top-0 left-0 w-full h-[300px] bg-gradient-to-b from-mosque/30 to-transparent -z-10" />

      <div className="w-full bg-mosque/20 border-b border-inch-worm/10 backdrop-blur-md sticky top-0 z-20">
        <div className="max-w-5xl mx-auto px-6 py-4">
          <div className="flex items-center gap-4 mb-2">
            <Link href="/xray" className="p-2 hover:bg-mosque/30 rounded-full transition-colors">
              <ArrowLeft className="h-5 w-5 text-white/60" />
            </Link>
            <h1 className="text-xl font-bold text-white font-heading">{execution.name}</h1>
            <span className={`px-2.5 py-0.5 rounded-full text-xs font-medium border ${execution.status === 'completed' ? 'bg-inch-worm/10 text-inch-worm border-inch-worm/20' : 'bg-red-500/10 text-red-400 border-red-500/20'}`}>
              {execution.status}
            </span>
          </div>
          <div className="flex items-center gap-6 text-sm text-white/50 ml-14">
          ...
          {steps?.map((step, index) => (
            <div key={step.id} className="relative pl-8 sm:pl-12 group">
              ...
              <div className="glass-panel rounded-xl overflow-hidden">
                <div className="bg-mosque/30 px-6 py-3 border-b border-inch-worm/10 flex justify-between items-center">
                  <h3 className="font-semibold text-white font-mono text-sm">{step.step_name}</h3>
                  <div className="flex items-center gap-3 text-xs text-white/50">
                    <span className="flex items-center gap-1"><Clock className="h-3 w-3" /> {step.duration_ms}ms</span>
                    {step.status === 'success' ? (
                      <span className="text-inch-worm font-medium flex items-center gap-1"><Check className="h-3 w-3" /> Success</span>
                    ) : (
                ...
                <div className="p-6">
                  {step.reasoning && (
                    <div className="mb-4 bg-mosque/30 border border-inch-worm/20 rounded-lg p-3">
                      <span className="text-xs font-bold text-inch-worm">REASONING: </span>
                      <span className="text-sm text-white/90">{step.reasoning}</span>
                    </div>
                  )}
                    ...
                    <>
                      ...
                      <div className="flex gap-3 mb-4">
                        <div className="flex-1 bg-white/5 rounded-lg p-2 text-center border border-white/5">
                          ...
                          <div className="text-xs text-white/50">Evaluated</div>
                        </div>
                        <div className="flex-1 bg-inch-worm/10 rounded-lg p-2 text-center border border-inch-worm/20">
                          <div className="text-lg font-bold text-inch-worm">{step.output.passed_count}</div>
                          <div className="text-xs text-inch-worm/70">Passed</div>
                        </div>
                  ...
                  {hasRankedOutput(step.output) && step.output.selected && (
                    <div className="space-y-4">
                      {/* Winner */}
                      <div className="bg-inch-worm/10 border border-inch-worm/30 rounded-xl p-4">
                        <div className="flex items-center gap-2 mb-2">
                          <Target className="h-5 w-5 text-inch-worm" />
                          <span className="text-sm font-bold text-inch-worm">SELECTED #1: </span>
                          <span className="text-white font-semibold">{step.output.selected.title}</span>
                          {step.output.selected.relevance_score && (
                            <span className="text-xs bg-inch-worm/20 text-inch-worm px-2 py-0.5 rounded">{step.output.selected.relevance_score}/100</span>
                          )}
                        </div>
                        {step.output.selected.reason_selected && (
                          <div className="ml-7 bg-inch-worm/5 border border-inch-worm/20 rounded-lg p-3 mt-2">
                            <span className="text-xs font-bold text-inch-worm">WHY SELECTED: </span>
                            <span className="text-sm text-inch-worm-light">{step.output.selected.reason_selected}</span>
                          </div>
                        )}
                      </div>
                      ...
                      {step.output.alternatives?.length > 0 && (
                        <div>
                          <h4 className="text-xs font-bold text-white/50 uppercase tracking-wider mb-3">Why Others Weren&apos;t #1</h4>
                          <div className="space-y-3">
                  ...
                  {!hasFilterEvaluations(step.output) && !hasLLMRelevance(step.output) && !hasRankedOutput(step.output) && (
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <div>
                        <h4 className="text-xs font-bold text-white/50 uppercase tracking-wider mb-2">Input</h4>
                        <div className="bg-mosque-dark rounded-lg border border-inch-worm/10 p-3">
                          <pre className="text-xs text-white/60 font-mono overflow-x-auto">{JSON.stringify(step.input, null, 2)}</pre>
                        </div>
                      </div>
                      <div>
                        <h4 className="text-xs font-bold text-white/50 uppercase tracking-wider mb-2">Output</h4>
                        <div className="bg-mosque-dark rounded-lg border border-inch-worm/10 p-3">
                          <pre className="text-xs text-white/60 font-mono overflow-x-auto">{JSON.stringify(step.output, null, 2)}</pre>
                        </div>
                      </div>
                    </div>
                  )}
